  ...
  return (
    <div className="p-6 space-y-5 max-w-[1400px] mx-auto">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => load(search, page, minReg)}
          disabled={loading}
          className="flex items-center gap-2 px-4 py-2 rounded-lg bg-stone-800 border border-stone-700 text-stone-300 hover:text-stone-100 hover:border-stone-600 text-sm transition-colors disabled:opacity-40"
        >
              ...
              <tbody className="divide-y divide-stone-800">
                {users.length === 0 ? (
                  ...
                  <tr key={user.id} className={`hover:bg-stone-800/40 transition-colors ${selected.has(user.id) ? 'bg-orange-500/5' : ''}`}>
                    ...
                    <td className="px-4 py-3">
                      <span className="text-stone-300 text-sm font-medium">{user.currentYearEventsCount ?? 0}</span>
                    </td>
                    <td className="px-4 py-3">
