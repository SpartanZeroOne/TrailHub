...
// ─── Nav item definitions (id + path only; labels come from i18n) ─────────────
const NAV_ITEMS = [
  {
  ...
  {
    id: 'organizers', path: '/admin/organizers',
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  {
    id: 'past-events', path: '/admin/past-events',
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  {
    id: 'csv-import', path: '/admin/csv-import',
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
// ─── Quick Search ─────────────────────────────────────────────────────────────
function QuickSearch({ onNavigate }) {
...
// ─── AdminLayout ──────────────────────────────────────────────────────────────
export default function AdminLayout({ children, currentPath, onNavigate, toasts, dismissToast, user }) {
  const { t } = useTranslation();
          ...
          <div className="px-4 py-3 border-t border-stone-800">
            <div className="flex items-center gap-2">
              ...
              <div className="min-w-0">
                ...
                <div className="text-xs text-stone-500">{t('nav.superAdmin')}</div>
              </div>
            </div>
          </div>
        )}
